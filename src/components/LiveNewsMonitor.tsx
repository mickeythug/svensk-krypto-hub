...
const LiveNewsMonitor = () => {
  ...
  const getRandomNewsText = () => {
    const newsTexts = ['📊 Trading volume spikes across major exchanges', '🏦 Major bank announces crypto custody services', '⚡️ Lightning Network reaches new payment milestone', '🌐 Layer 2 solutions see massive adoption surge', '💰 Institutional buying pressure continues to grow', '🔍 On-chain metrics show bullish divergence', '📈 Options market shows increased call buying', '🎯 Technical analysis points to potential breakout'];
  ...
  return <div className="fixed left-0 top-32 h-[calc(100vh-8rem)] w-80 bg-background/98 border-r border-border/80 backdrop-blur-md z-30 shadow-2xl">
      <div className="h-full flex flex-col font-jetbrains">
        ...
        <ScrollArea className="flex-1">
          <div className="p-4 space-y-3">
            {liveNews.map((item, index) => <Card key={item.id} className={`p-4 border-l-4 transition-all duration-300 hover:bg-secondary/30 cursor-pointer font-mono
                  ${item.priority === 'high' ? 'border-l-red-500 bg-red-50/10' : item.priority === 'medium' ? 'border-l-yellow-500 bg-yellow-50/10' : 'border-l-gray-400 bg-gray-50/10'}
