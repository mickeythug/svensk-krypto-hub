  ...
  return <div className="h-full w-full p-4 bg-gray-900/98 border border-gray-800/60 backdrop-blur-sm rounded-xl overflow-hidden">
      <div className="grid grid-cols-3 gap-6 h-full max-h-[500px]">
        ...
        <div className="space-y-4 h-full overflow-y-auto pr-2">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-gray-800/50">
            <div className="flex items-center gap-4">
              ...
              <Badge variant={priceChange24h >= 0 ? "default" : "destructive"} className="px-3 py-1 text-sm font-semibold">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant={side === 'buy' ? 'default' : 'outline'} onClick={() => setSide('buy')} className={`h-16 text-base font-bold transition-all duration-300 ${side === 'buy' ? 'bg-gradient-to-br from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white border-0 shadow-xl shadow-green-500/30' : 'bg-gray-800/60 border-gray-700/60 text-gray-300 hover:bg-gray-700/60 hover:border-gray-600/60'}`}>
            ...
            <Button variant={side === 'sell' ? 'default' : 'outline'} onClick={() => setSide('sell')} className={`h-16 text-base font-bold transition-all duration-300 ${side === 'sell' ? 'bg-gradient-to-br from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white border-0 shadow-xl shadow-red-500/30' : 'bg-gray-800/60 border-gray-700/60 text-gray-300 hover:bg-gray-700/60 hover:border-gray-600/60'}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-center h-12 bg-gray-800/60 border border-gray-700/60 rounded-lg px-3">
                <div className="flex items-center space-x-2">
                  <Switch checked={advancedMode} onCheckedChange={setAdvancedMode} className="data-[state=checked]:bg-primary" />
          ...
          <div className="space-y-3">
            ...
            <Input type="number" placeholder="0.00" value={amount} onChange={e => setAmount(e.target.value)} className="h-14 bg-gray-800/60 border-gray-700/60 text-white text-lg font-mono placeholder:text-gray-500 font-semibold tracking-wide" />
            
            {/* Quick Amount Buttons */}
            <div className="grid grid-cols-4 gap-2 mt-3">
              {quickAmounts.map(pct => <Button key={pct} variant="outline" size="sm" className="h-10 bg-gray-800/40 border-gray-700/40 text-white font-semibold hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-200" onClick={() => {
        ...
        <div className="space-y-4 h-full overflow-y-auto pr-2">
          ...
          {orderType === 'limit' && <div className="space-y-3">
              <Label className="text-white font-semibold text-sm tracking-wide">
                {t('trading.price')} (USDT)
              </Label>
              <Input type="number" placeholder="0.00" value={price} onChange={e => setPrice(e.target.value)} className="h-14 bg-gray-800/60 border-gray-700/60 text-white text-lg font-mono placeholder:text-gray-500 font-semibold tracking-wide" />
            </div>}

          {/* Stop Loss & Take Profit */}
          {advancedMode && <div className="space-y-3">
              <div className="space-y-4">
                <Label className="text-white font-semibold text-sm tracking-wide flex items-center gap-2">
                  <Shield className="h-4 w-4 text-red-400" />
                  Stop Loss (USDT)
                </Label>
                <Input type="number" placeholder="0.00" value={stopLoss} onChange={e => setStopLoss(e.target.value)} className="h-12 bg-red-900/20 border-red-700/60 text-white font-mono placeholder:text-gray-500 font-semibold" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Input type="number" placeholder="0.00" value={takeProfit} onChange={e => setTakeProfit(e.target.value)} className="h-12 bg-green-900/20 border-green-700/60 text-white font-mono placeholder:text-gray-500 font-semibold" />
              </div>
            </div>}
