...
const ModernStepTradingPanel: React.FC<ModernStepTradingPanelProps> = ({
  ...
  return (
    <Card className="h-full max-h-[400px] bg-gradient-to-br from-gray-900/95 to-gray-800/90 border border-emerald-500/20 p-4 flex flex-col overflow-hidden backdrop-blur-md"
      style={{ fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      
      {/* Header - Compact */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-bold text-emerald-400 truncate">{t('trading.trade')} {symbol}</h3>
          <p className="text-xs text-gray-400 font-mono truncate">${currentPrice.toFixed(2)}</p>
        </div>
        <div className="text-xs text-emerald-400/60 flex-shrink-0 ml-2">
          {currentChain || 'Ready'}
        </div>
      </div>
            ...
            <div key={step} className="flex items-center">
              ...
              {index < steps.length - 1 && (
                <div className={`w-8 h-0.5 mx-1 transition-all duration-200 ${
                  index < stepIndex ? 'bg-emerald-500' : 'bg-gray-700'
                }`} />
              )}
            </div>
          ))}
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 min-h-0">
        ...
        {currentStep === 'select' && (
          <div className="space-y-3">
            <h4 className="text-sm font-semibold text-emerald-400">1. {t('trading.orderType')}</h4>
            <div className="grid grid-cols-2 gap-2">
              <Button
                variant={side === 'buy' ? 'default' : 'outline'}
                onClick={() => setSide('buy')}
                className={`h-12 font-bold text-sm transition-all duration-200 ${
                  side === 'buy' 
                    ? 'bg-emerald-500 hover:bg-emerald-600 text-black shadow-lg shadow-emerald-500/25' 
                    : 'bg-gray-800/50 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-500/50'
                }`}
              >
              ...
              <Button
                ...
                className={`h-12 font-bold text-sm transition-all duration-200 ${
                  side === 'sell' 
                    ...
                    : 'bg-gray-800/50 border-red-500/30 text-red-400 hover:bg-red-500/10 hover:border-red-500/50'
                }`}
              >
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-medium text-gray-300">
                {side === 'buy' ? `Spend amount` : `Sell amount`} ({baseCurrency})
              </Label>
              <div className="relative">
                <Input
                  ...
                  className="bg-gray-900/70 border-emerald-500/20 text-emerald-400 text-sm h-10 pr-16 font-mono focus:border-emerald-500/50 focus:ring-emerald-500/20"
                />
              ...
              <div className="grid grid-cols-4 gap-1">
                {[25, 50, 75, 100].map((percent) => (
                  <Button
                    ...
                    className="bg-gray-800/30 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-500/40 text-xs h-7"
                  >
                    {percent}%
                  </Button>
                ))}
              ...
              <div className="text-xs text-gray-500 text-center font-mono">
                {t('trading.available')}: {
                  side === 'buy' 
                    ? `${mockBalances[baseCurrency.toLowerCase()].toFixed(2)} ${baseCurrency}`
                ...
                {[0.1, 0.5, 1.0].map((preset) => (
                  <Button
                    ...
                    className={`text-xs h-7 ${
                      slippage[0] === preset 
                        ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/25' 
                        : 'bg-gray-800/30 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10'
                    }`}
                  >
                    {preset}%
                  </Button>
          ...
          <>
            {!isFullyAuthenticated ? (
              <div className="space-y-3">
                ...
                <div className="bg-gray-900/70 rounded-lg p-3 space-y-2 border border-emerald-500/10">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('trading.orderType')}:</span>
                    <span className="text-emerald-400 font-medium capitalize">{side} {symbol}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('trading.amount')}:</span>
                    <span className="text-emerald-400 font-mono">{amount || '0'} {baseCurrency}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-emerald-400 font-mono">{slippage[0]}%</span>
                  </div>
                  <div className="border-t border-gray-700/50 pt-2">
                    <div className="flex justify-between font-semibold text-sm">
                      <span className="text-gray-300">{t('trading.total')}:</span>
                      <span className="text-emerald-400 font-mono">${calculateTotal().toFixed(4)}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-gray-900/70 rounded-lg p-3 space-y-2 border border-emerald-500/20">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">{t('trading.orderType')}:</span>
                    <span className="text-emerald-400 font-medium capitalize">{side} {symbol}</span>
                  </div>
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-emerald-400 font-mono">${fees.trading.toFixed(4)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-400">Network Fee:</span>
                    <span className="text-emerald-400 font-mono">${fees.network.toFixed(4)}</span>
                  </div>
                  {tip[0] > 0 && (
                    <div className="flex justify-between text-xs">
                      <span className="text-gray-400">{t('trading.tip')}:</span>
                      <span className="text-emerald-400 font-mono">${fees.tip.toFixed(4)}</span>
                    </div>
                  )}
                  <div className="border-t border-gray-700/50 pt-2">
                    <div className="flex justify-between font-semibold text-sm">
            ...
            <div className="space-y-2">
              {isLoading ? (
                <>
                  ...
                  <h4 className="text-sm font-semibold text-emerald-400">Executing Trade...</h4>
                  <p className="text-xs text-gray-400">Please confirm in your wallet</p>
                </>
              ) : (
                <>
                  <Check className="h-8 w-8 text-emerald-400 mx-auto" />
                  <h4 className="text-sm font-semibold text-emerald-400">Trade Executed!</h4>
                  <p className="text-xs text-gray-400">Your {side} order has been processed</p>
                </>
              )}
            </div>
          </div>
      ...
      <div className="flex gap-2 mt-3 pt-3 border-t border-gray-800/50">
        {currentStep !== 'select' && currentStep !== 'execute' && (
          <Button
            variant="outline"
            onClick={prevStep}
            className="flex-1 bg-gray-800/50 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10 text-xs h-9"
          >
        ...
        {currentStep === 'select' && (
          <Button
            onClick={nextStep}
            className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-black font-semibold text-xs h-9"
          >
            {t('common.continue')}
        ...
        {currentStep === 'amount' && (
          <Button
            ...
            className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-black font-semibold disabled:opacity-50 text-xs h-9"
          >
            {t('common.continue')}
            <ArrowRight className="h-3 w-3 ml-1" />
          </Button>
        ...
        {currentStep === 'summary' && !isFullyAuthenticated && (
          <Button
            onClick={() => {}}
            disabled={true}
            className="flex-1 bg-gray-600 text-gray-400 cursor-not-allowed text-xs h-9"
          >
            {t('trading.connectWalletMessage')}
          </Button>
        )}
