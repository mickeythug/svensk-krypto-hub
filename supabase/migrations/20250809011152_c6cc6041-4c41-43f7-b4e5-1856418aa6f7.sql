...
create policy if not exists "Market snapshots are viewable by everyone"
  on public.market_snapshots for select
  using (true);

create policy if not exists "Latest prices are viewable by everyone"
...
create trigger latest_token_prices_set_updated_at
